...
StartupEvents.registry("item", (e) => {
  ...
  global.preserves.forEach((jar) => {
    if (jar.item.includes("society")) {
      ...
      e.create(`homegrown:${jar.item.path}_bottle`)
        .texture(`homegrown:item/preserves/${jar.item.path}_bottle`)
        .food((food) => {
          food.hunger(2);
          food.saturation(0.6);
          food.fastToEat(true);
  ...
  global.jams.forEach((jar) => {
    if (jar.item.includes("homegrown")) {
      e.create(`homegrown:${jar.item.path}`)
        .texture(`homegrown:item/preserves/${jar.item.path}`);
    }
  });
  // Dehydrator outputs
