...
const handleMagicHarvest = (name, data, e) => {
  ...
  if (player.cooldowns.isOnCooldown(item)) return;
  if (["minecraft:sheep", "wildernature:minisheep", "dragnlivestock:o_sheep", "dragnlivestock:farm_goat", "dragnlivestock:o_goat"].includes(target.type)) handleSheepMagicShears(e);
  const affection = data.getInt("affection");
